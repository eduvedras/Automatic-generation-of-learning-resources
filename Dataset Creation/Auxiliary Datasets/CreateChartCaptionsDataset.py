    ...
    if filename.is_file():
        ...
        for image in images:
            ...
            if 'overfitting_decision_tree' in image:
                ...
                if file_tag == 'Iris':
                    first_var = 'PetalWidthCm <= 0.7'
                    second_var = 'PetalWidthCm <= 1.75'
                else:
                    new_row['description'] = f'An image showing a decision tree with depth = 2 where the first decision is made with the condition {first_var} and the second with the condition {second_var}.'
